...
class ReorderDragState(private val listState: LazyListState) {
    ...
    /**
     * Прокрутка списка на [delta] px в рамках кадра жеста — **вместе с компенсацией того
     * же кадра** (§5.4: «после прокрутки на `d` добавляет `d` к `offsetY`»).
     *
     * Компенсация вызывается здесь явно и не оставляется на layout-callback: вызывающий
     * сразу за этим пересчитывает цель, и она обязана считаться по уже компенсированному
     * смещению. Иначе правильность зависела бы от порядка callback'ов, а последний кадр у
     * края списка — тот самый, где прокрутка заканчивается, — мог бы остаться без
     * пересчёта вовсе.
     *
     * Двойной компенсации при этом не возникает: [syncSlotTop] идемпотентен — он сравнивает
     * верх слота с прошлым наблюдением, поэтому уже выполненная компенсация (callback
     * раскладки внутри `scrollBy`) второй раз не применяется.
     *
     * @return фактически потреблённое расстояние, px: у края списка оно меньше
     * запрошенного или нулевое, и именно по нему вызывающий решает, продолжать ли цикл.
     */
    suspend fun scrollBy(delta: Float): Float {
