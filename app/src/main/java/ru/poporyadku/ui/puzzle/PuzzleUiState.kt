 ...
 *
 * **Поднятости карточки здесь нет** (ITERATION_6_DESIGN.md, I6-D9): зарезервированное
 * итерацией 3 поле удалено. Признак «карточку держат пальцем» — локальное состояние
 * экрана (`ReorderDragState`), потому что состояние ViewModel переживает поворот и
 * пересоздание Activity, а палец на экране — нет: восстановленная «поднятая» карточка
 * прилипала бы к пустому месту.
 */
