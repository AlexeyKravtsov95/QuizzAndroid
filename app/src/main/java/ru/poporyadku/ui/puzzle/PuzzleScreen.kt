        ...
        snapshotFlow { dragState.autoScrollVelocity(edgeZonePx, maxVelocityPx) != NO_VELOCITY }
            .collectLatest { insideEdgeZone ->
                ...
                while (true) {
                    ...
                    // превращаться ни в рывок, ни в замедление прокрутки. На первом кадре
                    // цикла предыдущего времени ещё нет, поэтому `dt` нулевой и прокрутки
                    // в нём не происходит — это не край списка, а начало отсчёта.
                    val requested = velocity * elapsedSeconds
                    if (requested == NOTHING_REQUESTED) continue

                    // `scrollBy` компенсирует **потреблённое** расстояние в этом же кадре,
                    // поэтому цель ниже считается по уже актуальному смещению.
                    val consumed = dragState.scrollBy(requested)
                    // Цель пересчитывается после каждой прокрутки — в том числе после
                    // последней, когда список упёрся в край и потребил меньше
                    // запрошенного: карточка, пройденная этим кадром, обязана
                    // подтвердиться до остановки цикла.
                    dragState.emitTarget(confirmedOrder, onEvent)
                    // Список упёрся в край: продолжать нечем.
                    if (consumed == NOTHING_CONSUMED) break
                }
            }
    }
}
...
/** Список упёрся в край: запрошенная прокрутка не потреблена ни на пиксель. */
private const val NOTHING_CONSUMED = 0f

/** Кадр без прошедшего времени: прокручивать в нём нечего. */
private const val NOTHING_REQUESTED = 0f
