...
interface ExternalApps {
    ...
    /**
     * Отдать [text] в системный выбор приложения (PR 5D, §3.13).
     *
     * Доступность не спрашивается: выбор приложения разрешается системой всегда, поэтому
     * пары `canShareText` у этого действия нет. Отказ платформы — [LaunchResult.Failed]:
     * обработчик здесь не «исчез», как у ссылки или письма, а отказал сам системный
     * выбор.
     *
