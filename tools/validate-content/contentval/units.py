...
#: Идентификатор активного пакета приложения — `ContentPack.CORE_RU` и `@ActivePack`.
#:
#: CONTENT_MODEL.md §3 и ITERATION_4_DESIGN.md §4.2 требуют, чтобы `manifest.packId`
#: совпадал с ним, а §6.3 относит расхождение к коду `M02` «между манифестом и файлами
#: **либо с активным пакетом**». Значение известно инструменту так же, как известна
#: поддерживаемая версия формата: это константа проекта, а не аргумент запуска.
#: Самосогласованный пакет с чужим `packId` не импортируется приложением ни при каких
#: условиях, поэтому пропускать его в CI было бы обещанием, которого рантайм не выполнит.
ACTIVE_PACK_ID = "core-ru"
