...
Техническое проектирование итерации 5 «Архив, статистика, настройки, шеринг». Статус: **ревизия 1.2 (статусная), 2026-09-13; архитектура — ревизия 1.1, архитектурно утверждена 2026-09-11**. Итерация 5 технически реализована: PR 5A–5E влиты, PR 5E — PR №22 (merge `f96a5618c7174a35293d0b84406e296a6fd4f79d`). Ручные проверки `I5-M5`, `I5-M6`, `I5-M10` **не выполнены** и находятся в обязательном release-readiness gate итерации 7 (раздел 10.7, «Статус ручных проверок»). Технический Architecture Gate пройден; решения владельца O5-3, O5-4, O5-5, O5-6, O5-8, O5-9 подтверждены 2026-09-11, открытых архитектурных и продуктовых вопросов нет. O5-1, O5-2 и O5-7 — входные данные реализации, которые владелец предоставляет до мержа PR 5C и 5E (раздел 13.2). Реализация начинается с PR 5A.

**Что изменилось в ревизии 1.2** (статусная, 2026-09-13; архитектура, решения `I5-D*`, состав PR и тестовая матрица не менялись):

- **PR 5E влит** как PR №22 (merge `f96a5618c7174a35293d0b84406e296a6fd4f79d`); автоматические проверки PR №22 зелёные; итерация 5 технически реализована;
- **момент прохождения ручного gate изменён решением владельца от 2026-09-12**: `I5-M5` и `I5-M10`, которые по исходному критерию 14.2 (PR 5E) требовалось пройти до закрытия итерации 5, перенесены в обязательный release-readiness gate итерации 7; `I5-M6` находится там же с итерации 5B/5C. Ни одна из трёх проверок не выполнена и не объявляется выполненной; перенос не отменяет их и не снижает требований;
- исходные формулировки критериев (10.7, 14.2) сохранены без изменений для аудита; фактический статус добавлен отдельными примечаниями;
- `I4-C6` по-прежнему открыт в release-readiness итерации 7 (I5-D29).
...
- **статус утверждения** (2026-09-11): подтверждения владельца O5-3, O5-4, O5-5, O5-6, O5-8, O5-9 внесены в таблицу решений (3.0), обоснования (3.5, 3.7, 3.9, 3.13, 3.15), раздел 13 и критерии PR (14.2); O5-1, O5-2, O5-7 переведены во входные данные реализации; архитектура от этого не изменилась.
...
| Product flavors, signing, release-конфигурация, публикация | итерация 7 | для ссылки магазина фиксируется только нейтральный ресурс (раздел 3.13) |
...
Каждая строка закрыта решением `I5-D*` (раздел 3); решения, менявшие утверждённые документы или продуктовое поведение, подтверждены владельцем 2026-09-11 (раздел 13.1).
...
| 2 | `PRODUCT.md` §6 (контур A) против предпочтения задания | PRODUCT: «средний дневной балл — среднее по завершённым дням»; задание предлагает среднее по всем сыгранным дням | **I5-D5**, подтверждено владельцем (O5-3) |
...
| 7 | I3-D51 / I3-D12 против архивного итога | серия на `DayRecap` считается на `today`, а «Лучшая серия» появляется по признаку **этого** дня: для архивного дня экран показал бы сегодняшнее значение под рекордом прошлого | **I5-D9**, подтверждено владельцем (O5-5) |
| 8 | `COMPONENTS.md` (`SourcesBlock`: «отдельный маршрут для источников не создаётся») против `IMPLEMENTATION_PLAN.md` («экран источников») и `UX_FLOW.md` §8 («экран-список или раскрывающийся блок») | приоритет документов (`DESIGN_PRINCIPLES.md`, вводная) ставит план выше `COMPONENTS.md` | **I5-D14**, подтверждено владельцем (O5-4) |
...
Статусы решений — ровно два. «предложено» — решение предложено этим дизайном и принято вместе с архитектурным утверждением ревизии 1.1 (2026-09-11). «**подтверждено владельцем (2026-09-11)**» — решение меняет утверждённый документ или продуктовое поведение и подтверждено владельцем проекта (раздел 13.1). Неподтверждённых решений нет. Пометка «ресурс — O5-N» у принятого решения означает только, что конкретное значение ресурса владелец предоставляет до мержа указанного PR (раздел 13.2); само решение от значения не зависит.
...
| I5-D14 | Настройки — один плоский экран: «Звук», «Вибрация», группа «Тема» из трёх строк с радиокнопками, раздел «О приложении» в том же списке, строка «Источники» → отдельный маршрут `sources`, строка «Сообщить о неточности». Напоминания нет | источники встроенным `SourcesBlock` в настройках; «О приложении» отдельным экраном | **подтверждено владельцем (2026-09-11)**, O5-4 |
...
| I5-D18 | Письмо строит чистый `ReportMailComposer` из шаблонов ресурсов; `mailto:`-URI кодирует чистый `MailtoUri` (RFC 6068, UTF-8, `%20`, `%0D%0A`); получатель — ресурс `feedback_email` | `Uri.Builder`/`URLEncoder` (даёт `+` вместо пробела) | предложено; ресурс — O5-1: адрес владелец предоставляет до мержа 5C |
...
| I5-D21 | Ссылка на приложение — нейтральный непереводимый ресурс `share_app_url` в `res/values/distribution.xml`; в итерации 5 product flavors не вводятся; значение на время альфы — `https://poporyadku.invalid/`; release input итерации 7 — реальный URL универсальной landing-страницы, ведущей в RuStore и Huawei AppGallery | `RU_STORE_URL`; URL в коде builder; разные значения ресурса в distribution-сборках RuStore и AppGallery | **подтверждено владельцем (2026-09-11)**, O5-6 |
...
| I5-D23 | Звук — `SoundPool` (`USAGE_ASSISTANCE_SONIFICATION`) с двумя OGG-ресурсами, объект живёт в `ActivityRetainedComponent` и освобождается ровно один раз при окончательном уничтожении Activity; `OnLoadCompleteListener` регистрируется до первого `load()`, звук играет только после `status == 0`; вибрация — `View.performHapticFeedback` без разрешения `VIBRATE` | `Vibrator`/`VibratorManager`; `ToneGenerator`; экранный `SoundPool` | предложено; ресурс — O5-7: два OGG-файла и подтверждение их лицензии владелец предоставляет до мержа 5E |
...
| I5-D26 | Все новые пользовательские тексты — в `strings.xml`; строки заглушек `stub_*` удаляются; предлагаемые формулировки, которых нет в утверждённых документах, перечислены в разделе 13.3 | литералы в Compose-коде | **подтверждено владельцем (2026-09-11)**, O5-8 |
...
| I5-D30 | Новые визуальные элементы (строка архива, `notPlayed` и интерактивность `DayResultRow`, пометка отзыва, группа темы, раздел «О приложении», экран источников) добавляются в `COMPONENTS.md` state sheets до кода соответствующего PR | импровизация в коде | **подтверждено владельцем (2026-09-11)**, O5-9 |
...
2. **Частичный балл незавершённого дня в среднее не входит.** Предпочтение задания (среднее по всем сыгранным дням) отклонено; среднее по завершённым дням подтверждено владельцем 2026-09-11 (O5-3). Обоснование:
...
**Почему источники — отдельный маршрут, а «О приложении» — раздел (подтверждено владельцем 2026-09-11, O5-4).** `COMPONENTS.md` закрепляет встроенный `SourcesBlock` в настройках и запрещает маршрут; `IMPLEMENTATION_PLAN.md` (итерация 5) говорит «экран источников», `UX_FLOW.md` §8 допускает оба. По приоритету документов (`DESIGN_PRINCIPLES.md`, вводная) план выше `COMPONENTS.md`. Технические доводы за маршрут:
...
«О приложении» остаётся разделом списка: это шесть коротких строк, `UX_FLOW.md` §8 требует плоский список, а отдельный экран ради них был бы вложенным экраном без собственного состояния.
...
| Автор | подпись автора | ресурс `about_author`; **значение — входные данные владельца O5-2, до мержа 5C** |
...
| Обратная связь | адрес, выделяемый и копируемый | ресурс `feedback_email`; **значение — входные данные владельца O5-1, до мержа 5C** |
...
- получатель — ресурс `feedback_email` (`translatable="false"`); значение — входные данные владельца **O5-1**. Выдумывать адрес дизайн не вправе: пока адрес не получен, PR 5C не мержится (код и тесты от значения не зависят).
...
- Ресурс называется нейтрально — `share_app_url` — и лежит в отдельном файле `res/values/distribution.xml` с `translatable="false"`: в итерации 7 в этом файле меняется только значение — на реальный URL универсальной landing-страницы (O5-6); код и остальные ресурсы не затрагиваются.
...
- **Значение в итерации 5 — `https://poporyadku.invalid/`** (подтверждено владельцем 2026-09-11, O5-6). Домен `.invalid` зарезервирован RFC 2606 и не бывает продуктовым: заведомо нерабочая ссылка допустима только на время альфы, `COMPONENTS.md` допускает плейсхолдер до релиза, реального адреса дизайн не выдумывает.
- **Release input итерации 7** (подтверждено владельцем 2026-09-11, O5-6): одна универсальная landing-страница владельца, которая ведёт в RuStore и Huawei AppGallery: один ресурс на все магазины, ради ссылки flavors не нужны и в итерации 7, карточка не устаревает при смене магазина. Реальный URL landing-страницы — release input итерации 7. Release-readiness итерации 7: `.invalid` в release-сборке запрещён; `share_app_url` содержит реальный URL универсальной landing-страницы; landing-страница ведёт в RuStore и Huawei AppGallery.
- Старая формулировка `{RU_STORE_URL}` в `UX_FLOW.md` §6, `COMPONENTS.md` («ShareCard»), `UI_REVIEW_CHECKLIST.md` и `b2/state-sheets/share-card-format.md` заменяется на `{APP_SHARE_URL}` / `share_app_url` в PR итерации 5 (раздел 12).
...
- Формулировки, которых нет в утверждённых документах, собраны в разделе 13.3, приняты владельцем как предложено 2026-09-11 (O5-8) и вносятся в `UX_FLOW.md` правками раздела 12.
...
`today` из сигнатуры убран: при подтверждённом решении O5-5 (серия этого дня) он нужен только заголовку, и его читает ViewModel (I3-D51). Зависимости use case после итерации 5: `DayAssignmentRepository`, `PuzzleRepository`, `ProgressRepository`.
...
**Статус ручных проверок на 2026-09-13 (ревизия 1.2).** Исходная колонка «PR» выше сохранена как проектное назначение. Фактически:

| ID | Статус | Где закрывается |
| --- | --- | --- |
| I5-M5 | **не выполнена** — физического устройства при приёмке PR 5E не было; перенесена решением владельца от 2026-09-12 | обязательный release-readiness gate итерации 7, физическое устройство |
| I5-M6 | **не выполнена** — живой TalkBack требует физического устройства; перенесена решением владельца | тот же gate |
| I5-M10 | **не выполнена** — перенесена решением владельца от 2026-09-12 | тот же gate |

Перенос меняет только момент прохождения gate: требования проверок не снижены, автоматические тесты (`I5-N8`, `I5-F2`, `I5-V29`…`I5-V34`) и эмулятор их не заменяют, и ни одна из них не отмечается пройденной без записи устройства, версии Android и результата.
...
| 5C | Адрес или подпись автора не получены | PR 5C нельзя смержить | O5-1, O5-2 — входные данные раздела 13.2 со сроком «до мержа 5C»; код и тесты 5C от значений не зависят |
| 5D | Альфа-значение `.invalid` доживает до публичного релиза | получатель карточки из публичной версии видит нерабочую ссылку | `https://poporyadku.invalid/` одобрен владельцем только для альфы (O5-6); release-readiness итерации 7 запрещает `.invalid` в release-сборке и требует реальный URL универсальной landing-страницы, ведущей в RuStore и Huawei AppGallery |
...
Дизайн утверждён 2026-09-11; правки выполняются в указанных PR итерации 5 и входят в их критерии готовности. В дизайн-PR ни один из этих документов не меняется.
...
| `SourcesBlock` | «в настройках не используется; источники по сыгранным — отдельный экран `sources`» (O5-4) | 5C |
...
| `docs/IMPLEMENTATION_PLAN.md` | итерация 7: **release inputs** — `share_app_url` — реальный URL универсальной landing-страницы, ведущей в RuStore и Huawei AppGallery, вместо альфа-значения `https://poporyadku.invalid/` (O5-6), `feedback_email`, `about_author`, запрет `.invalid`, `I4-C6` на физическом устройстве, лицензии звуков | 5C, 5D |
| `docs/IMPLEMENTATION_PLAN.md` | итерация 5 — статус «выполнена» с фактами приёмки | 5E |
| `docs/PRODUCT.md` | §6 контур B и `R-6`: «RuStore Console» → «консоли RuStore и Huawei AppGallery» (публикация минимум в два магазина). §6 контур A не меняется: владелец подтвердил среднее по завершённым дням (O5-3) | 5D |
...
## 13. Решения владельца и входные данные реализации

Владелец проекта подтвердил 2026-09-11 шесть решений: O5-3, O5-4, O5-5, O5-6, O5-8, O5-9 (раздел 13.1). Открытых архитектурных и продуктовых решений по итерации 5 нет; реализация может начинаться с PR 5A. O5-1, O5-2 и O5-7 — не открытые вопросы, а обязательные входные данные: владелец предоставляет их до мержа PR 5C и 5E (раздел 13.2). Код и тесты от этих значений не зависят — тесты подставляют собственные строки и фейковый `SoundEngine`.

### 13.1 Подтверждённые решения

| ID | Подтверждённое решение | Дата |
| --- | --- | --- |
| O5-3 | Средний балл — по завершённым дням (I5-D5), как `PRODUCT.md` §6; частичный балл незавершённого дня в среднее не входит | 2026-09-11 |
| O5-4 | Источники — отдельный экран `sources` (I5-D14); встроенный `SourcesBlock` в настройках не используется | 2026-09-11 |
| O5-5 | Исторический итог дня показывает серию выбранного дня (`streakAtDay`, I5-D9); «Лучшая серия» при `isRecordUpdated` — то же значение | 2026-09-11 |
| O5-6 | `share_app_url` на время альфы — `https://poporyadku.invalid/`; в итерации 7 — одна универсальная landing-страница, ведущая в RuStore и Huawei AppGallery (I5-D21); реальный URL — release input итерации 7 | 2026-09-11 |
| O5-8 | Все тексты раздела 13.3 приняты как предложено (I5-D26) | 2026-09-11 |
| O5-9 | Дополнения Design Gate и новые компоненты (I5-D30) приняты: `Archive row` в две строки, `DayResultRow.notPlayed` и интерактивность в архиве, `RetiredNotice` первой строкой результата, тема тремя строками-радиокнопками, «О приложении» разделом списка, композиция экрана источников | 2026-09-11 |

### 13.2 Входные данные владельца

| ID | Что должен предоставить владелец | Срок | Что блокирует |
| --- | --- | --- | --- |
| O5-1 | Адрес электронной почты для обратной связи — значение ресурса `feedback_email` (получатель письма «Сообщить о неточности», строка «О приложении») | до мержа 5C | мерж PR 5C |
| O5-2 | Подпись автора для «О приложении» — значение ресурса `about_author` | до мержа 5C | мерж PR 5C |
| O5-7 | Два звуковых файла OGG по спецификации раздела 3.14 (`res/raw/feedback_move.ogg`, `res/raw/feedback_accept.ogg`) и подтверждение их происхождения и лицензии для `docs/VERSIONS.md` | до мержа 5E | мерж PR 5E |

Дизайн эти значения не придумывает: не подставляет личный адрес автора без явного согласия, не выбирает подпись и не создаёт звуки. Начало реализации, PR 5A, 5B и 5D от входных данных не зависят; разработка 5C и 5E до их получения тоже возможна — ждёт только мерж.

Release input итерации 7 (не входные данные итерации 5): реальный URL универсальной landing-страницы для `share_app_url` (O5-6). Release-readiness итерации 7 запрещает `.invalid` в release-сборке и требует, чтобы landing-страница вела в RuStore и Huawei AppGallery.

### 13.3 Новые пользовательские тексты

Строки, взятые дословно из утверждённых документов, помечены источником; строки с пометкой «предложение» — новые формулировки дизайна, приняты владельцем как предложено 2026-09-11 (O5-8).
...
| PR | Готов, когда |
| --- | --- |
| **5A** | тесты `I5-S1…S9` (включая расширенный `I5-S8`: границы `set_index` и счёт по числу закрытых слотов), `I5-A1…A10` зелёные; проверки mapper стоят до вычисления `dayNumber`; существующие `GetTodayStateUseCaseTest` и `HomeViewModelTest` проходят без изменения утверждений; `AppDatabase.version == 1`, в `app/schemas` нет второго файла; в `ui/**` нет изменений; `ARCHITECTURE.md` (§1, §3, ADR-017), `IMPLEMENTATION_PLAN.md` (статус итерации 4, разбиение итерации 5), `ITERATION_4_DESIGN.md` («Ход реализации») синхронизированы |
| **5B** | тесты `I5-R1…R6`, `I5-V1…V18`, `I5-V35`, `I5-C1…C11` зелёные; `I5-N1…N3`, `I5-N5…N7` скомпилированы и пройдены вручную на эмуляторе; тесты I3 итога и результата (`I3-C10…C13`, `I3-C15`, `I3-C17`, `I3-U38…U41`) проходят с обновлёнными типами, `I3-V34` переписан под `GetDayRecapUseCase(localDate)`; существующие тесты `SubmitAnswerUseCaseTest` проходят с новой зависимостью; `rg -n -e setHasCompletedFirstDay -e updateStreakCache -e GetStreaksUseCase app/src/main/java/ru/poporyadku/ui/recap app/src/main/java/ru/poporyadku/domain/usecase/GetDayRecapUseCase.kt` пусто; `rg -nF "setHasCompletedFirstDay(" app/src/main/java/ru/poporyadku/ui app/src/main/java/ru/poporyadku/domain/usecase` находит только `domain/usecase/SubmitAnswerUseCase.kt`; `rg -n -e ArchiveStubScreen -e SampleArchiveDate -e stub_archive app/src` пусто; `rg -nF "Destinations.puzzle(" app/src/main/java/ru/poporyadku/ui/archive app/src/main/java/ru/poporyadku/ui/recap` пусто; `when` по `SlotOutcome`/`SlotResultUi` исчерпывающие без `else`; `I5-M6…M9` для архива пройдены; документы 5B раздела 12 синхронизированы |
| **5C** | тесты `I5-T1`, `I5-T2`, `I5-E1…E5`, `I5-Q1`, `I5-Q2`, `I5-V19…V27`, `I5-C12…C14`, `I5-C16`, `I5-P1` зелёные, причём `I5-V20…V22` и `I5-V24` выполняются на настоящей `SettingsWriteQueue` с `TestScope`; `rg -n NonCancellable app/src/main/java/ru/poporyadku/ui/settings app/src/main/java/ru/poporyadku/data/prefs` пусто; `rg -nF -e "setSoundEnabled(" -e "setVibrationEnabled(" -e "setThemeMode(" app/src/main/java/ru/poporyadku/ui` пусто — UI пишет настройки только через `SettingsWriter`; `I5-N4` пройден вручную; `I5-M2…M4`, `I5-M6…M9` для настроек и источников пройдены; в манифесте добавлен ровно один `<intent>` в `<queries>` (`SENDTO mailto`) и ни одного разрешения; `rg -n "startActivity" app/src/main/java/ru/poporyadku/ui` находит только `ui/platform/AndroidExternalApps.kt`; `rg -n -e Stub -e stub_ app/src/main` пусто; строки «Напоминание» в настройках нет; ресурсы `feedback_email` и `about_author` содержат реальные значения, предоставленные владельцем (O5-1, O5-2), — без них PR 5C не мержится; документы 5C раздела 12 синхронизированы |
| **5D** | тесты `I5-H1…H10`, `I5-V28`, `I5-C15`, `I5-P2` зелёные; `I5-M1` пройден; `rg -n -i -e rustore -e appgallery app/src/main/java` пусто; `ShareCardBuilder.kt` не импортирует `android.*` и не содержит `@Preview`; `share_app_url` лежит в `distribution.xml` и равен одобренному для альфы `https://poporyadku.invalid/` (O5-6); документы 5D раздела 12 синхронизированы; release-readiness итерации 7, не критерий 5D: `.invalid` в release-сборке запрещён, `share_app_url` содержит реальный URL универсальной landing-страницы, которая ведёт в RuStore и Huawei AppGallery |
| **5E** | тесты `I5-F1`, `I5-F2` (включая порядок listener → load, независимость двух файлов, ошибочный `status`, однократный `release()`), `I5-V29…V34` зелёные; `I5-N8` и `I5-M5`, `I5-M10` пройдены; `rg -n -e VIBRATE -e Vibrator app/src/main` пусто; `rg -n "SoundPool" app/src/main/java` находит только `ui/feedback/SoundEngine.kt`; литералов пользовательского текста в `ui/**` вне `@Preview` нет (сверка по `rg -n "Text\(\s*\"" app/src/main/java/ru/poporyadku/ui` и `rg -n "contentDescription = \"" app/src/main/java/ru/poporyadku/ui` — пусто); неиспользуемых строк в `strings.xml` нет (`lint` `UnusedResources` без предупреждений по новым строкам); два OGG-файла и подтверждение их происхождения и лицензии получены от владельца (O5-7), `docs/VERSIONS.md` дополнен; все строки раздела 12 выполнены; `IMPLEMENTATION_PLAN.md` отмечает итерацию 5 выполненной, `I4-C6` — открытым пунктом итерации 7 |

**Примечание ревизии 1.2 к строке 5E.** Критерий «`I5-N8` и `I5-M5`, `I5-M10` пройдены» сохранён в исходной формулировке. PR 5E влит как PR №22 по решению владельца от 2026-09-12, которое перенесло `I5-M5` и `I5-M10` в release-readiness итерации 7; обе проверки на момент мержа **не выполнены**. Остальные пункты критерия 5E выполнены автоматикой PR №22. Так же читаются исторические критерии 5B и 5C «`I5-M6…M9` … пройдены»: их часть `I5-M6` (живой TalkBack) не выполнена и перенесена владельцем в тот же gate итерации 7 (раздел 10.7, «Статус ручных проверок»).
...
| 1.1 | 2026-09-11 | Точечные исправления по ревью: загрузка `DayRecap` без записей DataStore, `hasCompletedFirstDay` — в `SubmitAnswerUseCase` (новое `I5-D31`, `I5-R6`, `I5-V35`); порядок `SoundPool` listener → load и однократный `release()` через `SoundCueBank`/`SoundEngine` (`I5-D23`, `I5-F2`); записи настроек — application-scoped очередь с одним worker и ошибками по ключам, без `NonCancellable` (`I5-D15`, `I5-V20`…`I5-V22`, `I5-V24`); строгий mapper строки архива (`I5-S8`); синхронизированы файлы и критерии PR 5A, 5B, 5C, 5E, риски и план правок документов. Архитектура итерации и состав PR 5A–5E не менялись. По итогам финального ревью (2026-09-11) внесены подтверждения владельца O5-3, O5-4, O5-5, O5-6, O5-8, O5-9; O5-1, O5-2, O5-7 переведены во входные данные реализации. Статус — архитектурно утверждена |
| 1.2 | 2026-09-13 | Статусная ревизия без изменений архитектуры: PR 5E влит как PR №22 (merge `f96a5618c7174a35293d0b84406e296a6fd4f79d`), итерация 5 технически реализована; решение владельца от 2026-09-12 перенесло ручные `I5-M5` и `I5-M10` в обязательный release-readiness gate итерации 7, где уже находится `I5-M6`; все три проверки не выполнены; исходные критерии 10.7 и 14.2 сохранены, фактический статус добавлен примечаниями; `I4-C6` остаётся открытым |

---

**Статус документа: ревизия 1.2 (статусная), 2026-09-13; архитектура — ревизия 1.1, архитектурно утверждена 2026-09-11.** Итерация 5 технически реализована: PR 5A–5E влиты (последний — PR №22). Ручные проверки `I5-M5`, `I5-M6`, `I5-M10` не выполнены и находятся в обязательном release-readiness gate итерации 7; `I4-C6` не выполнен и находится там же.

*Историческая строка статуса ревизии 1.1 (2026-09-11), сохранена без изменений:* **Статус документа: ревизия 1.1, архитектурно утверждена, 2026-09-11.** Технический Architecture Gate пройден. Решения O5-3, O5-4, O5-5, O5-6, O5-8, O5-9 подтверждены владельцем; открытых архитектурных вопросов нет. O5-1, O5-2 и O5-7 остаются входными данными реализации (до мержа PR 5C и 5E). Реализация может начинаться с PR 5A; повторное архитектурное ревью перед PR 5A не требуется. Реализация итерации 5 не начата; ни один Kotlin-файл, ресурс, манифест, Room-схема, JSON-контент, Gradle-скрипт, CI-конфигурация и ни один другой документ этим документом не изменены.
