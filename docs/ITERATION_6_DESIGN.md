...
Техническое проектирование итерации 6 «Перетаскивание, доступность, визуальная полировка» и напоминаний. Статус: **ревизия 1.0, готова к архитектурному ревью, 2026-09-13.** Документ не утверждён; решения `I6-D*` имеют статус «предложено», решения владельца `O6-*` (раздел 18) открыты.
...
| 14 | Критерий `rg -n "android\.permission" app/src/main/AndroidManifest.xml` пуст (итерация 5) | `POST_NOTIFICATIONS` утверждён продуктом; WorkManager вливает свои разрешения в итоговый манифест | **I6-D41**, **O6-7** |
| 15 | `COMPONENTS.md` (`Settings row`): отказ в разрешении — только строка-подсказка | после постоянного отказа системный диалог не появляется, и подсказка — единственный выход без действия | **O6-4** |
...
| I6-D5 | Жестовые события: `DragStarted(cardId)`, `DragMovedTo(cardId, targetIndex)`, `DragFinished(cardId)`; `DragMoved(from, to)` и `DragEnded` удаляются | сохранить контракт I3 | 4.3 |
...
| I6-D7 | Порог: центр перетаскиваемой карточки пересекает центр соседней; цель — индекс карточки, в границы которой попал центр; один кадр может пройти несколько позиций — это одна перестановка | фиксированное расстояние в dp | 5.3 |
...
| I6-D9 | Сессия перетаскивания во ViewModel — только в памяти (`DragSession(cardId, startIndex)`); `PuzzleBoard.draggedCardId` удаляется из состояния, «поднятость» — локальное состояние UI без `rememberSaveable` | хранить `draggedCardId` в `PuzzleUiState` | 4.5 |
...
| I6-D21 | `DragEducationHint` — в 6C по `COMPONENTS.md`; видимость — `Playing.showDragHint` во ViewModel; таймеры 4000/600 мс — в UI по токенам `dragHint.*`; флаг — команда `SettingMutation.DragHintSeen` через use case | подсказка в 6A | 7.9 |
...
| I6-D24 | Напоминания: доменная граница `domain/reminder` (чистые расчёты, интерфейсы `ReminderScheduler`, `ReminderNotifier`, `NotificationAccess`, use cases); Android-реализации — `notifications/`; `notifications` зависит от `domain`, ни `ui`, ни `data` не импортирует | Android-код в `domain`; worker, вызывающий репозитории напрямую | 9.1 |
...
| I6-D26 | Одна уникальная работа `daily_reminder`, `OneTimeWorkRequest` без ограничений; синхронизация настроек — `ExistingWorkPolicy.REPLACE`; перепланирование самим worker'ом — `APPEND_OR_REPLACE`; выключение — `cancelUniqueWork` | `PeriodicWorkRequest`; `KEEP` | 9.3 |
...
| I6-D29 | `ReminderScheduleSynchronizer` в `@ApplicationScope` наблюдает `(reminderEnabled, reminderTime)` с `distinctUntilChanged` и последовательно планирует/отменяет; запускается один раз на процесс из `MainActivity.onCreate`; вызов `resync()` — от приёмника смены времени | вызовы планировщика из `SettingsViewModel`; запуск из `Application.onCreate` | 9.4 |
| I6-D30 | Приёмник `ReminderTimeChangeReceiver` в манифесте на `TIME_SET` и `TIMEZONE_CHANGED` (исключения из запрета неявных broadcast), `exported="false"`, вызывает `resync()` | только самокоррекция worker'а | 9.5 |
...
| I6-D36 | Разрешение: переключатель «Напоминание» при отсутствии доступа на API 33+ запускает системный запрос; отказ — переключатель выключен, подсказка (и действие — **O6-4**); API 26–32 с выключенными уведомлениями — подсказка без запроса; различение «отказ»/«постоянный отказ» не требуется и новый флаг не заводится | `shouldShowRequestPermissionRationale` + флаг «уже спрашивали» | 8.2 |
| I6-D37 | Экран показывает переключатель как `reminderEnabled && доступ есть`, пересчитывая доступ на `ON_START`; отзыв разрешения вне приложения ничего не пишет | автоматически записывать `false` при отзыве | 8.1 |
...
| I6-D39 | `SettingMutation` расширяется: `ReminderEnabled`, `ReminderTime`, `NotificationPromptShown`, `DragHintSeen`; очередь та же; экран настроек показывает ошибки только ключей `Reminder`, `ReminderTime` | отдельная очередь напоминаний | 8.1 |
| I6-D40 | Предложение на `DayRecap` — отдельный `ReminderPromptViewModel` в том же route-контейнере: `DayRecapViewModel` по-прежнему не инжектирует настройки (I5-D31); условие — сессионный вариант, день завершён, `!notificationPromptShown`, `!reminderEnabled`; флаг пишется до системного запроса | поля диалога в `DayRecapViewModel` | 8.3 |
| I6-D41 | Сети нет: ни `INTERNET`, ни сетевых вызовов; итоговый манифест проверяется тестом на точный набор разрешений; судьба `ACCESS_NETWORK_STATE`, вливаемого WorkManager, — **O6-7** | не проверять итоговый манифест | 10.4 |
...
| I6-D49 | Ошибки планирования и показа не видны пользователю и не влияют на игровой поток: синхронизатор и worker перехватывают всё, кроме `CancellationException` | ошибка на экране настроек | 9.8 |
...
sealed interface PuzzleEvent {
    ...
    data class DragMovedTo(val cardId: String, val targetIndex: Int) : PuzzleEvent
    /** И отпускание, и отмена, и потеря указателя: поведение одинаково (I6-D8). */
    data class DragFinished(val cardId: String) : PuzzleEvent
...
private data class DragSession(val cardId: String, val startIndex: Int)
...
fun onEvent(event: PuzzleEvent) = when (event) {
    ...
    is PuzzleEvent.DragStarted -> onDragStarted(event.cardId)
    is PuzzleEvent.DragMovedTo -> onDragMovedTo(event.cardId, event.targetIndex)
    is PuzzleEvent.DragFinished -> onDragFinished(event.cardId)
...
private fun onDragStarted(cardId: String) {
    val playing = state.value as? PuzzleUiState.Playing ?: return
    val index = playing.board.cards.indexOfFirst { it.cardId == cardId }
    if (index < 0) return
    if (dragSession?.cardId == cardId) return                 // повтор события — без второй отдачи
    dragSession = DragSession(cardId, index)
    emitFeedback(FeedbackCue.CardGrabbed)
    // 6C: скрыть DragEducationHint, если показана
}

private fun onDragMovedTo(cardId: String, targetIndex: Int) {
    if (dragSession?.cardId != cardId) return                 // перемещение без захвата не принимается
    reorder(cardId, MoveTarget.Index(targetIndex), MoveOrigin.Drag)
}

private fun onDragFinished(cardId: String) {
    val session = dragSession?.takeIf { it.cardId == cardId } ?: return
    dragSession = null
    val playing = state.value as? PuzzleUiState.Playing ?: return
    val order = playing.board.cards.map { it.cardId }
...
| Во время жеста нет объявлений; при завершении — одно | `origin == Drag` | `I6-V4` |
...
Сессия нужна ровно для двух вещей: не отдавать `CardGrabbed` дважды за один захват и объявить результат жеста один раз. Поэтому она не сохраняется: после поворота, пересоздания Activity или смерти процесса пальца на экране уже нет, и восстановленная сессия означала бы «поднятую» карточку без жеста. Висящая сессия безвредна: следующий `DragStarted` заменяет её, кнопочное действие закрывает, `DragFinished` с чужим `cardId` игнорируется.
...
| `Submitting.Answer` | есть, тот же вид | не принимается: `pointerInput` пересоздаётся по ключу `interactive = false`, активный жест отменяется, `DragFinished` уходит и игнорируется ViewModel (состояние не `Playing`) | disabled | отсутствуют |
...
### 5.3 Визуальное смещение против подтверждённой перестановки (I6-D6, I6-D7)
...
class ReorderDragState(private val listState: LazyListState) {
    ...
    /** Смещение пальца относительно текущего слота карточки, px. Только визуал. */
    var offsetY: Float by mutableFloatStateOf(0f); private set
    /* start(cardId), drag(deltaY) → targetIndex?, finish() */
}
```

Алгоритм кадра:

1. `onDrag(delta)`: `offsetY += delta.y`.
2. Центр перетаскиваемой карточки = `itemInfo.offset + offsetY + itemInfo.size / 2` в координатах списка.
3. `DragTargetResolver.targetIndex(cards, draggedIndex, draggedCenter)` — чистая функция над списком `(cardIndex, top, bottom)` **только карточек** (элементы `prompt` и подсказки исключаются по ключу): цель — индекс карточки, в чьи `[top, bottom)` попал центр; ниже последней — последний индекс, выше первой — `0`. Пока центр не пересёк центр соседа, цель равна текущему индексу.
4. Если цель ≠ текущему индексу — `onEvent(DragMovedTo(cardId, target))`.
5. После перекомпозиции (новый подтверждённый порядок пришёл из ViewModel) элемент карточки переехал в новый слот; UI компенсирует `offsetY -= (новый offset слота − старый)`, поэтому карточка остаётся под пальцем без скачка.

| Вопрос | Ответ |
| --- | --- |
| Источник истины во время жеста | `PuzzleUiState.Playing.board.cards` (ViewModel). UI не хранит своего порядка |
| Источник истины после жеста | тот же; UI обнуляет `offsetY` и `draggedCardId` |
...
| Гистерезис | встроен в правило «центр пересёк центр соседа»: после перестановки центр соседа оказывается по другую сторону на половину высот, дребезга на границе нет (`I6-R3`) |
| Несколько позиций за жест | да — последовательными подтверждениями; быстрый рывок через две карточки за кадр — одно подтверждение на две позиции (одна отдача) |
...
| Реализация | корутина в `LaunchedEffect(draggedCardId)`: `while (активна) { withFrameNanos { dt -> listState.scrollBy(v × dt) } ; пересчитать цель по шагу 3 5.3 }` |
...
| --- | --- | --- | --- |
| Палец отпущен | `finish()`, `DragFinished(cardId)` | объявление, если позиция изменилась | последний подтверждённый порядок |
| `ACTION_CANCEL`, системный жест, звонок | `onDragCancel` → то же | то же | то же; отката нет |
| Уход с экрана во время жеста (назад, навигация) | композиция уходит, корутины отменяются, событие может не дойти | сессия висит в памяти до смерти ViewModel | порядок в `SavedStateHandle` уже подтверждён |
| Поворот / пересоздание Activity | `ReorderDragState` не saveable — карточка не «прилипает» | ViewModel жива, сессия висит и безвредна (4.5) | порядок сохранён, повторной отдачи нет |
| Смерть процесса | — | восстановление `orderOf()` | последний подтверждённый порядок, отдачи нет |
...
| Флаг | при показе ViewModel один раз вызывает `MarkDragHintSeenUseCase` → `SettingsWriter.submit(SettingMutation.DragHintSeen)`; повторного показа нет даже при смерти процесса во время показа |
...
| «Напоминание» | `Settings row` с `Switch`; вся строка — `toggleable(role = Switch)` | значение = `reminderEnabled && notificationAccess.isAllowed()` (**I6-D37**) |
...
| Подсказка отказа | `bodySmall` под строкой «Напоминание»: «Разрешите уведомления в настройках системы» (`UX_FLOW.md` §8) + действие «Открыть настройки уведомлений» (**O6-4**) | видна, когда последняя попытка включения упёрлась в отсутствие доступа либо `reminderEnabled == true`, а доступа нет |
...
`SettingsState` получает `reminder: ReminderUi?` (`null` до первой эмиссии — skeleton), `SettingsEvent` — `ReminderToggled(enabled)`, `ReminderTimeChosen(LocalTime)`, `NotificationPermissionResult(granted)`, `OpenNotificationSettingsClicked` (если **O6-4** принят); `SettingsEffect` — `RequestNotificationPermission`, `OpenNotificationSettings`. Доступ пересчитывается на каждом `ON_START` через `SettingsViewModel.onScreenStarted()`. Команды записи — только из `onEvent` (как I5-D15), автоматических записей при изменении доступа нет.

### 8.2 Разрешение (I6-D36)

| Ситуация | Что происходит |
| --- | --- |
| Переключатель включают, доступ есть (API 26–32 и уведомления приложения включены; API 33+ и разрешение выдано; канал не заблокирован) | команда `ReminderEnabled(true)`; синхронизатор планирует работу |
| API 33+, разрешение не выдано | запись не выполняется; эффект `RequestNotificationPermission`; route-контейнер запускает `ActivityResultContracts.RequestPermission()` для `POST_NOTIFICATIONS`; цель запроса хранится в `SavedStateHandle` ViewModel (переживает пересоздание во время системного диалога) |
| Системный диалог — «Разрешить» | `NotificationPermissionResult(true)` → `ReminderEnabled(true)` |
| «Не разрешать» или диалог закрыт | `NotificationPermissionResult(false)` → записи нет, переключатель остаётся выключенным, видна подсказка |
| Постоянный отказ (система больше не показывает диалог) | запрос возвращается сразу с `false` — тот же путь, та же подсказка; приложение не пытается распознать «навсегда» и не заводит флаг |
| API 26–32, уведомления приложения выключены в системе | системного запроса нет; переключатель остаётся выключенным, видна подсказка |
| Канал `daily_reminder` отключён пользователем | `isAllowed() == false` — как выключенные уведомления |
| Разрешение отозвано вне приложения | на следующем `ON_START` настроек переключатель показан выключенным (`UX_FLOW.md` §11); `reminderEnabled` не переписывается; worker не показывает уведомления без доступа |
| Разрешение снова выдано в системе | переключатель снова показан включённым, напоминания возобновляются — пользователь его не выключал |
| Повторный запрос | только по новому явному нажатию; автоматических повторов нет |
| Выключение | `ReminderEnabled(false)`; синхронизатор отменяет работу |
| Смена времени | `ReminderTime(t)` по подтверждению; синхронизатор перепланирует |

`NotificationAccess.isAllowed()` (`notifications/AndroidNotificationAccess`) = `NotificationManagerCompat.areNotificationsEnabled()` **и** (API < 33 **или** `checkSelfPermission(POST_NOTIFICATIONS) == GRANTED`) **и** важность канала ≠ `IMPORTANCE_NONE` (если канал создан).
...
| «Да, в 9:00» | 1) `NotificationPromptShown` (до всего остального); 2) доступ есть → `ReminderTime(09:00)`, `ReminderEnabled(true)`; API 33+ без разрешения → системный запрос; «Разрешить» → те же две команды; отказ → ничего не включается, без текста-упрёка |
| «Не нужно», системная «назад» | только `NotificationPromptShown`; системного запроса нет |
| Касание вне диалога | ничего |
| Повторный показ | нет: флаг записан до запроса; ViewModel скрывает диалог сразу, не дожидаясь эмиссии DataStore |
...
| `settings_reminder_open_system` | Открыть настройки уведомлений | предложение (**O6-4**) | 6B |
...
```
domain/reminder/                          // чистый Kotlin + coroutines, без android.*
  ...
  ReminderScheduler.kt                    // interface { fun schedule(t: ReminderTrigger, mode: ScheduleMode); fun cancel() }
  ReminderNotifier.kt                     // interface { fun show(); fun cancelShown() }
  NotificationAccess.kt                   // interface { fun isAllowed(): Boolean }
  EvaluateReminderUseCase.kt              // решение перед показом (только чтение)
  ReminderRun.kt                          // оркестрация одного срабатывания worker'а
  ReminderScheduleSynchronizer.kt         // наблюдение настроек → планировщик
  GetReminderPromptEligibilityUseCase.kt  // условие диалога на DayRecap
notifications/                            // Android: WorkManager, NotificationManager, Receiver
  WorkManagerReminderScheduler.kt         // над узкой границей UniqueWorkOperations (enqueue/cancel) — фейк в I6-P1
  ReminderWorker.kt                       // CoroutineWorker: EntryPoint → ReminderRun
  ReminderWorkerEntryPoint.kt             // @EntryPoint @InstallIn(SingletonComponent)
  AndroidReminderNotifier.kt              // канал, показ, снятие
  AndroidNotificationAccess.kt
  ReminderTimeChangeReceiver.kt
di/ReminderModule.kt                      // @Binds интерфейсов domain/reminder
```

Зависимости: `notifications → domain → core.model`; `ui` видит `domain/reminder` (use cases, `NotificationAccess`) и не видит `notifications`, кроме `MainActivity` (корень композиции: запуск синхронизатора, создание канала, снятие показанного уведомления — через интерфейсы из Hilt). `domain/reminder` не импортирует `androidx.work`, `android.app.Notification*`.

**Зависимость.** В `app/build.gradle.kts` добавляется `implementation(libs.androidx.work.runtime.ktx)`; строка catalog уже существует. Версия 2.11.2 зафиксирована 2026-08-30 — в начале 6B она сверяется со страницей релизов WorkManager и записывается в `VERSIONS.md` с датой проверки (правило итерации 0: ни одной версии по памяти). `androidx.hilt:hilt-work` не нужен: `ReminderWorker` получает граф через `EntryPointAccessors.fromApplication(applicationContext, ReminderWorkerEntryPoint::class.java)` — это API уже подключённого `hilt-android`. `work-testing` не нужен: логика — в `ReminderRun` и чистых объектах (JVM), адаптер WorkManager — в instrumented-тесте на настоящем WorkManager (`I6-N6`). Если в 6B выяснится, что без новой зависимости критерий не проверяем, это возвращается владельцу отдельным решением, а не добавляется молча.
...
| Планирование из синхронизатора (`ScheduleMode.Replace`) | `enqueueUniqueWork(name, REPLACE, request)` — отменяет ожидающую и выполняющуюся работу; новые настройки важнее |
| Перепланирование из worker'а (`ScheduleMode.AfterCurrent`) | `enqueueUniqueWork(name, APPEND_OR_REPLACE, request)` — пока текущий worker выполняется, следующая работа ставится после него и не отменяет его; если цепочка уже завершилась, заменяет |
...
### 9.4 Синхронизатор (I6-D29)

```kotlin
@Singleton
class ReminderScheduleSynchronizer @Inject constructor(
    ...
    private val resyncs = MutableSharedFlow<Unit>(extraBufferCapacity = 1)

    fun start() {
        if (!started.compareAndSet(false, true)) return
        scope.launch {
            combine(
                preferences.preferences.map { it.reminderEnabled to it.reminderTime }.distinctUntilChanged(),
                resyncs.onStart { emit(Unit) },
            ) { schedule, _ -> schedule }
                .collect { (enabled, time) -> apply(enabled, time) }   // строго последовательно
        }
    }

    fun resync() { resyncs.tryEmit(Unit) }

    private fun apply(enabled: Boolean, time: LocalTime) {
        try {
            if (!enabled) scheduler.cancel()
            else { val now = clock.now(); scheduler.schedule(NextReminderTrigger.next(Instant.ofEpochMilli(now.epochMillis), now.zone, time), ScheduleMode.Replace) }
        } catch (e: CancellationException) { throw e } catch (e: Exception) { /* I6-D49: повтор при следующем старте/изменении */ }
    }
}
```
...
| Вопрос | Ответ |
| --- | --- |
| Кто вызывает планировщик при изменении настроек | только синхронизатор; `SettingsViewModel` и `ReminderPromptViewModel` пишут DataStore через `SettingsWriter` и о WorkManager не знают |
| Конкурентные включение/выключение/смена времени | команды записи упорядочены очередью `SettingsWriteQueue`; синхронизатор получает эмиссии DataStore по порядку и вызывает `enqueue`/`cancel` последовательно; операции WorkManager выполняются его последовательным исполнителем в порядке вызова — итог соответствует последней записи (`I6-Y1`) |
| Идемпотентность | повтор с теми же настройками заменяет работу той же целью; двух работ не бывает — имя уникально |
| Старт приложения | `MainActivity.onCreate` вызывает `start()`; первая эмиссия пересчитывает цель от текущего момента и зоны. Запуск не из `Application.onCreate`: Robolectric-тесты создают `PoPoRyadkuApp` без инициализатора WorkManager, и обращение к нему упало бы во всех тестах |
| Перезапуск процесса WorkManager'ом без Activity | синхронизатор не нужен: цепочку продолжает сам worker (`AfterCurrent`) |
| Зависит ли от разрешения | нет: работа планируется по намерению; без доступа worker завершается без показа и перепланирует следующий день |
...
| Событие | Реакция |
| --- | --- |
| Смена часового пояса (`ACTION_TIMEZONE_CHANGED`) | `ReminderTimeChangeReceiver` → `EntryPoint` → `synchronizer.resync()` (через `goAsync()`, завершение после вызова) |
| Ручной перевод часов (`ACTION_TIME_CHANGED`, `android.intent.action.TIME_SET`) | то же |
| Приложение не запущено в момент события | оба действия входят в список исключений из ограничения неявных broadcast — приёмник из манифеста получает их и поднимает процесс |
...
| Force stop | система снимает задачи приложения до следующего ручного запуска — **ограничение платформы, а не обещание приложения**; первый запуск `MainActivity` перепланирует |
| Home при смене даты в открытом приложении | O-2 итерации 3 закрыт без приёмника: минутный тикер и `ON_START` (I3-D15) |

`exported="false"` у приёмника: системные broadcast доставляются от системного uid и такому приёмнику; факт проверяется в 6B на эмуляторе (`I6-M7`). Если доставка не подтвердится, допустимая замена — `exported="true"` с теми же двумя защищёнными системными действиями; другие изменения решения не требуются.
...
class ReminderRun @Inject constructor(
    ...
    /** Никогда не бросает, кроме CancellationException. */
    suspend operator fun invoke(scheduledDate: LocalDate, scheduledMinute: Int) {
        var next: ReminderTrigger? = null
        try {
            val verdict = evaluate(scheduledDate, scheduledMinute)
            next = verdict.nextTrigger                      // null — напоминание выключено
            if (verdict is ReminderVerdict.Show) notifier.show()
        } catch (e: CancellationException) {
            throw e
        } catch (e: Exception) {
            next = evaluate.fallbackNextTrigger()           // по текущим настройкам, без чтения базы; null — выключено
        } finally {
            next?.let { scheduler.schedule(it, ScheduleMode.AfterCurrent) }   // не выполняется при отмене
        }
    }
}
```
...
| 5 | `NotificationAccess.isAllowed()` | `Skip(NoAccess)` | `next(now)` (завтра) |
...
`ReminderWorker.doWork()`: разбирает `inputData` (неразбираемые данные → `Result.success()` без показа и один `synchronizer.resync()`); вызывает `ReminderRun`; возвращает `Result.success()` всегда — повтор с backoff не нужен, следующая попытка уже запланирована. `CancellationException` пробрасывается.
...
- Синхронизатор и `ReminderRun` перехватывают `Exception`, кроме `CancellationException`; пользователь ошибок планирования не видит, игровой поток от них не зависит.
- Отказ `enqueue` в синхронизаторе: работа не запланирована до следующего изменения настроек, `resync()` или старта `MainActivity`.
...
- Отмена worker'а системой: `CancellationException` пробрасывается, перепланирования нет — работу отменил тот, кто поставил новую.
...
<application …>
    ...
    <!-- O6-7: удаление влитого WorkManager разрешения, если принято -->
</application>
```
...
| `ACCESS_NETWORK_STATE` (ожидаемо) | WorkManager, для сетевых ограничений | **O6-7**: рекомендация — удалить `tools:node="remove"`, у напоминания нет сетевых ограничений; удаление подтверждается `I6-N6` на эмуляторе API 35 и на эмуляторе с API ниже 33 (системный образ ставится через `sdkmanager`) |
| `INTERNET` | — | отсутствует; тест проверяет |
...
| Ошибки планирования не ломают игровой поток | **I6-D49**, `I6-Y1`, `I6-W3` |
...
| Пересоздание Activity во время перетаскивания | карточка не остаётся поднятой; сессия ViewModel висит безвредно | `I6-N2` |
...
| `ui/puzzle/DragTargetResolver.kt` | чистый расчёт цели по границам карточек |
...
| --- | --- |
| `ui/puzzle/PuzzleEvent.kt`, `PuzzleUiState.kt` | 4.3; удаление `draggedCardId` |
| `ui/puzzle/PuzzleViewModel.kt` | `reorder()`, `DragSession`, обработчики жеста |
...
**Зависит от:** `main`; ответов **O6-1**, **O6-2**, **O6-3**, **O6-4**, **O6-7**; входных данных «иконка уведомления» (18.2) — до мержа. Разработка до ответов возможна: код не зависит от текстов и формы времени.

| Создаётся | Назначение |
| --- | --- |
| `domain/reminder/*` (9.1) | расчёты, интерфейсы, use cases, `ReminderRun`, синхронизатор |
| `notifications/*` (9.1) | WorkManager, worker, уведомление, доступ, приёмник |
...
| тесты JVM: `NextReminderTriggerTest`, `ReminderEligibilityTest`, `EvaluateReminderUseCaseTest`, `ReminderRunTest`, `ReminderScheduleSynchronizerTest`, `ReminderPromptViewModelTest` | `I6-S*`, `I6-W*`, `I6-Y1`, `I6-V15` |
| тесты Robolectric: `testDebug/…/notifications/WorkManagerReminderSchedulerTest.kt`, `AndroidReminderNotifierTest.kt`, `AndroidNotificationAccessTest.kt`, `ReminderTimeChangeReceiverTest.kt`, `ManifestPermissionsTest.kt`; `testDebug/…/ui/components/NotificationOptInDialogTest.kt` | `I6-P1`…`I6-P5`, `I6-C10` |
...
| `AndroidManifest.xml` | 10.3; **O6-7** |
| `MainActivity.kt` | `synchronizer.start()`, `ensureChannel()`, снятие показанного в `onStart` |
...
| `ui/platform/ExternalApps.kt`, `AndroidExternalApps.kt` | `openNotificationSettings()` при принятом **O6-4** |
...
**Тесты:** `I6-S1`…`I6-S4`, `I6-W1`…`I6-W4`, `I6-Y1`, `I6-V14`…`I6-V16`, `I6-C9`…`I6-C11`, `I6-P1`…`I6-P6`, `I6-N6`; `I6-K1`…`I6-K3`, `I6-K8`. **Ручные:** `I6-M5` (эмулятор), `I6-M6`, `I6-M7`, `I6-M8`, `I6-M9`, `I6-M13`.

**Готов, когда:** тесты зелёные; `I6-N6` и ручные проверки пройдены на эмуляторе API 35 и эмуляторе API ниже 33 с записью API и результата в описании PR; итоговый манифест совпадает с утверждённым набором разрешений; `VERSIONS.md` содержит сверенную версию WorkManager с датой; `DayRecapViewModel` не изменён; тексты и иконка уведомления получены от владельца.
...
**Запрещено в diff:** `app/schemas`, `assets`, CI, `ui/puzzle`, `ui/components/OrderableCard.kt`, `DayRecapViewModel.kt`, `INTERNET`, сетевые библиотеки, `hilt-work`, `work-testing`, `SCHEDULE_EXACT_ALARM`, `AlarmManager`.
...
| I6-R3 | `DragTargetResolver`: до пересечения центра соседа цель прежняя; после — соседний индекс; скачок через две карточки за один сэмпл — индекс +2; выше первой — 0, ниже последней — 3; элементы `prompt`/`drag_hint` игнорируются; последовательность сэмплов у границы не даёт чередования целей | `test/…/ui/puzzle/DragTargetResolverTest` | 6A | I6-D7 |
...
| I6-V5 | `DragStarted`: `Feedback(CardGrabbed)` с `performHaptic = true`, `playSound = false`; вибрация выключена или `Unknown` — эффекта нет; повтор `DragStarted` той же карточки — без второй отдачи | `PuzzleViewModelTest` | 6A | I6-D14 |
...
| I6-V9 | Новый ViewModel на `SavedStateHandle` после перестановок жестом (имитация восстановления процесса): порядок восстановлен, эффектов отдачи и объявлений нет, `DragMovedTo` без нового `DragStarted` игнорируется | `PuzzleViewModelTest` | 6A | I6-D9 |
| I6-V10 | Неизвестный `cardId`, `DragFinished` без сессии, `DragFinished`/`DragMovedTo` с чужим `cardId`: ничего не происходит | `PuzzleViewModelTest` | 6A | I6-D5 |
...
| I6-N2 | Эмулятор: частичный жест, затем `ActivityScenario.recreate()`: карточка не поднята, порядок = последнее подтверждение, после пересоздания отдач нет | `PuzzleDragFlowTest` | 6A | I6-D9 |
...
| I6-V13 | Подсказка во ViewModel: `hasSeenDragHint = false` → `showDragHint = true` и ровно одна команда `DragHintSeen`; скрывается первой перестановкой любого источника, `DragStarted`, `DragHintDismissed`; при `true` не показывается; повторная загрузка не отправляет вторую команду | `PuzzleViewModelTest` | 6C | I6-D21 |
...
| I6-W3 | `CancellationException` из `peek()` пробрасывается, показа и перепланирования нет; `IllegalStateException` — перехвачена | `ReminderRunTest` | 6B | I6-D49 |
...
| I6-Y1 | Синхронизатор: старт с выключенным → `cancel`; включение → `schedule(Replace)` с целью `NextReminderTrigger`; последовательность вкл → выкл → вкл → смена времени обрабатывается строго по порядку, последний вызов соответствует последнему состоянию; эмиссия другого ключа — без вызова; `resync()` → повторный `schedule`; второй `start()` — без второго сбора; исключение планировщика не останавливает сбор | `test/…/domain/reminder/ReminderScheduleSynchronizerTest` | 6B | I6-D29 |
| I6-V14 | `SettingsViewModel`: `reminder` = `enabled && allowed`; включение с доступом → команда `ReminderEnabled(true)`; без доступа на API 33+ → эффект запроса, записи нет; результат `true` → команда; `false` → записи нет, подсказка; API 26–32 без доступа → подсказка без запроса; выключение → `ReminderEnabled(false)`; выбор времени → `ReminderTime`; `onScreenStarted` после отзыва → показано выключенным, записей нет; цель запроса переживает новый экземпляр ViewModel на том же `SavedStateHandle`; ошибки ключей `Reminder`/`ReminderTime` видны, `DragHintSeen`/`NotificationPromptShown` — нет | `test/…/ui/settings/SettingsViewModelTest` | 6B | I6-D36…I6-D39 |
| I6-V15 | `ReminderPromptViewModel`: показ только при сессии + завершённом дне + `!promptShown` + `!reminderEnabled`; архив, незавершённый день, флаг, включённое напоминание — нет; «Да» → сначала `NotificationPromptShown`, затем при доступе `ReminderTime(09:00)` и `ReminderEnabled(true)`, без доступа — эффект запроса; результат `true` → две команды, `false` → ничего; «Не нужно»/«назад» → только флаг, без запроса; после любого ответа диалог скрыт сразу; отказ чтения условий → диалога нет; `CancellationException` пробрасывается | `test/…/ui/recap/ReminderPromptViewModelTest` | 6B | I6-D40 |
...
| I6-P3 | `AndroidNotificationAccess`: API 33 без разрешения → `false`; с разрешением → `true`; API 32 с включёнными уведомлениями → `true`; уведомления приложения выключены → `false`; канал `IMPORTANCE_NONE` → `false` | `testDebug/…/notifications/AndroidNotificationAccessTest` | 6B | I6-D36 |
| I6-P4 | `ReminderTimeChangeReceiver`: `TIMEZONE_CHANGED` и `TIME_SET` → один `resync()`; прочие действия — ничего; итоговый манифест: приёмник с ровно двумя действиями и `exported = false` | `testDebug/…/notifications/ReminderTimeChangeReceiverTest` | 6B | I6-D30 |
| I6-P5 | Итоговый манифест (`PackageManager.getPackageInfo(GET_PERMISSIONS)`): запрошенные разрешения равны утверждённому набору (10.4, **O6-7**); `INTERNET` отсутствует | `testDebug/…/ManifestPermissionsTest` | 6B | I6-D41 |
| I6-P6 | `AndroidExternalApps.openNotificationSettings()`: `Settings.ACTION_APP_NOTIFICATION_SETTINGS` с `EXTRA_APP_PACKAGE`; защита от повторного запуска и перехват исключений — как `I5-P1` (только при принятом **O6-4**) | `testDebug/…/ui/platform/AndroidExternalAppsTest` | 6B | I6-D36 |
| I6-C9 | Настройки: группа «Напоминание» с `heading()`; строка с ролью `Switch`; строка времени есть только при показанном включении; подсказка отказа (и действие по **O6-4**) под строкой; ошибка записи под своей строкой; `w320dp-h844dp` при 200 % — всё видно, горизонтальной прокрутки нет; тёмная тема | `testDebug/…/ui/settings/SettingsScreenTest` | 6B | 8.1 |
...
| I6-N6 | Эмулятор, настоящий WorkManager: `schedule(Replace)` → одна работа `ENQUEUED` с данными; повтор → по-прежнему одна; `cancel` → `CANCELLED`; при удалённом `ACCESS_NETWORK_STATE` (**O6-7**) worker без ограничений выполняется без `SecurityException` | `androidTest/…/notifications/ReminderWorkSchedulingTest` | 6B | I6-D26, I6-D41 |
...
| I6-M6 | Разрешение: выдать, отказать, отказать повторно до постоянного отказа (API 33+, эмулятор API 35); выключенные уведомления приложения на эмуляторе с API ниже 33; отзыв разрешения в системных настройках и возврат в «Настройки» | эмулятор — для мержа; физическое устройство — итерация 7 | 6B | итерация 7 |
| I6-M7 | Смена часового пояса и ручной перевод часов при закрытом и открытом приложении: работа перепланирована на местное время новой зоны (`adb shell dumpsys jobscheduler`/`WorkManager` diagnostics), приёмник с `exported="false"` получил broadcast | эмулятор | 6B | — |
...
| Влитые WorkManager разрешения противоречат обещанию «без сети» | тест точного набора; решение **O6-7** | `I6-P5`, `I6-M13` | `tools:node="remove"` или revert 6B |
...
| §1, дерево и правила зависимостей | `domain/reminder/*`; фактический `notifications/*`; `notifications → domain` (реализует интерфейсы и вызывает use cases; `ui` и `data` не импортирует) | 6B |
...
| §6 | полностью по разделам 8–10: проверки перед показом, отсутствие перепланирования «при завершении дня», канал, приёмник смены времени, force stop | 6B |
...
| §8, «Напоминание» | группа, строки, эффективное значение, подсказка и действие (**O6-3**, **O6-4**), тексты 8.6 после **O6-2** | 6B |
...
| `NotificationOptInDialog` | точные условия показа (8.3); флаг до системного запроса | 6B |
| `Settings row` | группа «Напоминание», строка времени, эффективное значение, подсказка и действие (**O6-4**), форма выбора времени (**O6-3**) | 6B |
...
До ответов на решения ниже статус документа остаётся «ревизия 1.0, готова к архитектурному ревью». Чисто технические решения, следующие из утверждённой архитектуры, сюда не вынесены.
...
| O6-4 | Действие при отказе в разрешении | строка-подсказка **и** текстовое действие «Открыть настройки уведомлений» (`ACTION_APP_NOTIFICATION_SETTINGS` через `ExternalApps`): после постоянного отказа это единственный путь, не требующий искать настройки вручную | только подсказка, как в `COMPONENTS.md` сейчас | мерж 6B |
...
| O6-7 | Разрешение `ACCESS_NETWORK_STATE`, вливаемое WorkManager, при критерии итерации 7 «нет ни одного сетевого разрешения» | **Удалить** `tools:node="remove"`: сетевых ограничений у напоминания нет, в карточке магазина не будет сетевого разрешения; условие — зелёный `I6-N6` | оставить с пояснением, что оно не даёт доступа в сеть, и уточнить формулировку критерия итерации 7 | мерж 6B |
...
**Статус документа: ревизия 1.0, готова к архитектурному ревью, 2026-09-13.** Решения `I6-D*` предложены и не утверждены; решения владельца `O6-1`…`O6-7` открыты; входные данные раздела 18.2 не получены. Реализация итерации 6 не начата. `I4-C6`, `I5-M5`, `I5-M6`, `I5-M10` не выполнены и остаются пунктами release-readiness итерации 7.
