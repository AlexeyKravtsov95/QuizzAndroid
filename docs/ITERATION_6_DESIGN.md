...
Техническое проектирование итерации 6 «Перетаскивание, доступность, визуальная полировка» и напоминаний. Статус: **ревизия 1.2, архитектурно утверждена, 2026-09-15.** Решения `I6-D*` приняты вместе с архитектурным утверждением; решения владельца `O6-1`…`O6-6` подтверждены владельцем 2026-09-15 (раздел 18).
...
Статус всех решений — «принято»: они приняты вместе с архитектурным утверждением этого документа (ревизия 1.2, 2026-09-15). Решения, требовавшие ответа владельца, вынесены в раздел 18, помечены ссылкой `O6-*` и подтверждены владельцем 2026-09-15.
...
| Краевая зона | токен `dragAutoScroll.edgeZone` — ссылка на `size.touchTarget.min` (48 dp), без нового числа (**O6-6**, подтверждено владельцем (2026-09-15)) |
| Скорость | `v = maxVelocity × (глубина захода / edgeZone)`, глубина ограничена `edgeZone`; токен `dragAutoScroll.maxVelocity` — выводится из существующих (**O6-6**, подтверждено владельцем (2026-09-15)): одна карточка с зазором за `motion.duration.long`, (`size.orderableCard.minHeight` + `spacing.listGap`) / `motion.duration.long` ≈ 413 dp/с (**O6-6**) |
...
| Текст | «Три новых задания готовы» — **O6-2**, подтверждено владельцем (2026-09-15) |
...
| День начат и закрыт хотя бы один слот | **O6-1**, подтверждено владельцем (2026-09-15): не показывать |
...
Строки с источником взяты дословно из утверждённых документов; строки с пометкой **O6-*** подтверждены владельцем 2026-09-15; строки с пометкой «предложение» — новые формулировки, отдельно владельцем не подтверждены и сверяются до мержа своего PR (18.2).

| Ресурс | Текст | Источник | PR |
| --- | --- | --- | --- |
| `settings_group_reminder` | Напоминание | **O6-3**, подтверждено владельцем (2026-09-15) | 6B |
...
| `settings_reminder_permission_hint` | Разрешите уведомления в настройках системы | `UX_FLOW.md` §8, `COMPONENTS.md`; единый текст для всех причин — **O6-4**, подтверждено владельцем (2026-09-15) | 6B |
| `settings_reminder_open_system` | Открыть настройки уведомлений | **O6-4**, подтверждено владельцем (2026-09-15) (действие обязательно по **I6-D36**) | 6B |
| `reminder_time_confirm` / `reminder_time_cancel` | Сохранить / Отмена | **O6-3**, подтверждено владельцем (2026-09-15) | 6B |
...
| `reminder_channel_name` | Напоминание о заданиях | **O6-2**, подтверждено владельцем (2026-09-15) | 6B |
| `reminder_channel_description` | Одно напоминание в день в выбранное время | **O6-2**, подтверждено владельцем (2026-09-15) | 6B |
| `reminder_notification_text` | Три новых задания готовы | **O6-2**, подтверждено владельцем (2026-09-15) (вместо «Новые три задания готовы» из `ARCHITECTURE.md` §6) | 6B |
...
| `Assigned` | `completedCount` 1–2 | не показывать — **O6-1**, подтверждено владельцем (2026-09-15) |
...
Токены подтверждены владельцем 2026-09-15 (**O6-5**, **O6-6**); в код они попадают только после внесения в `DESIGN_TOKENS.md` в своём PR.
...
**Зависит от:** `main` на `f96a561`; утверждения этого документа; **O6-6** (токены auto-scroll) подтверждено владельцем (2026-09-15).
...
| `ui/theme/Sizing.kt`/`Motion.kt` | только токены auto-scroll (**O6-6**), после внесения в `DESIGN_TOKENS.md` |
...
**Зависит от:** `main`; **O6-1**…**O6-4** подтверждено владельцем (2026-09-15); входных данных «иконка уведомления» и сверки оставшихся строк-предложений 8.6 (18.2) — до мержа.
...
**Зависит от:** 6A, 6B, 6C; **O6-5** подтверждено владельцем (2026-09-15); входные данные «иконка приложения».
...
| I6-S4 | `ReminderEligibility`: все строки таблицы 9.6 (`NewSet`, `CarryOver`, `Assigned` с 0/1/2/3 закрытыми, `AwaitingNextDay`, `ContentExhausted`); строка «1–2» — не показывать (**O6-1**) | `test/…/domain/reminder/ReminderEligibilityTest` | 6B | I6-D33 |
...
Все решения ниже подтверждены владельцем 2026-09-15 в рекомендованных вариантах; документ — «ревизия 1.2, архитектурно утверждена, 2026-09-15». Чисто технические решения, следующие из утверждённой архитектуры, сюда не вынесены.

### 18.1 Решения владельца (подтверждены 2026-09-15)

| ID | Вопрос | Подтверждённый вариант | Отклонённые альтернативы | Что было заблокировано |
| --- | --- | --- | --- | --- |
| O6-1 | Показывать ли напоминание, если сегодняшний день начат и закрыто 1–2 задания | **подтверждено владельцем (2026-09-15):** **Не показывать**: пользователь уже вернулся сам, а напоминание о недоигранном дне читается как давление за серию (`PRODUCT.md` §4, R-9) | показывать с отдельным нейтральным текстом | мерж 6B (`I6-S4`) |
| O6-2 | Тексты канала и уведомления (8.6) | **подтверждено владельцем (2026-09-15):** канал «Напоминание о заданиях» / «Одно напоминание в день в выбранное время»; уведомление «По порядку!» + «Три новых задания готовы» (грамматически естественнее текста `ARCHITECTURE.md` §6) | дословно «Новые три задания готовы»; иная формулировка владельца | мерж 6B |
| O6-3 | Композиция напоминания в настройках и форма выбора времени | **подтверждено владельцем (2026-09-15):** отдельная группа «Напоминание» после «Звук и вибрация»; выбор времени — **встроенный M3 `TimeInput`** под строкой «Время» с «Сохранить»/«Отмена», без диалога: не появляется третий компонент с тенью (`DESIGN_PRINCIPLES.md` §6) | M3 `TimePicker` в диалоге (новое исключение для тени и формы); системный `TimePickerDialog` (чужой визуальный язык) | мерж 6B; правка `COMPONENTS.md` |
| O6-4 | Тексты подсказки при недоступности уведомлений (само действие перехода в системные настройки обязательно по **I6-D36**) | **подтверждено владельцем (2026-09-15):** один общий текст для всех причин: «Разрешите уведомления в настройках системы» + действие «Открыть настройки уведомлений» — пользователю не нужно различать разрешение, уведомления приложения и канал | отдельные тексты для `RuntimePermissionMissing`, `AppNotificationsDisabled` и `ChannelDisabled` | мерж 6B |
| O6-5 | Тип перехода между экранами и появление результата | **подтверждено владельцем (2026-09-15):** переход — **только затухание/проявление** (fade) на токенах `DESIGN_PRINCIPLES.md` §9; reveal — **группами**, не больше 8 шагов (≤ 520 мс) | сдвиг по оси (shared axis) — больше движения, чем требует спокойный продукт; построчный reveal без предела | мерж 6D; правка `DESIGN_TOKENS.md` |
| O6-6 | Токены auto-scroll | **подтверждено владельцем (2026-09-15):** `dragAutoScroll.edgeZone` = `size.touchTarget.min`; `dragAutoScroll.maxVelocity` = одна карточка с зазором за `motion.duration.long` — оба производные, без новых чисел | собственные значения владельца | мерж 6A |
...
| Тексты 8.6 | тексты **O6-2**, **O6-3**, **O6-4** подтверждены 2026-09-15; сверить оставшиеся строки-предложения (`settings_reminder_time`, `settings_reminder_time_value`, `cd_settings_reminder_time`) | до мержа 6B | мерж 6B |
...
| Иные визуальные решения | нет: **O6-3**, **O6-5**, **O6-6** подтверждены 2026-09-15 | — | — |
...
| 1.2 | 2026-09-15 | Финальные исправления ревью: приёмник смены времени вызывает `finish()` только после долговечного результата — сохранения уникальной работы `reminder_resync` (`ReminderResyncWorker` → `SyncReminderScheduleUseCase`) либо подтверждённой прямой синхронизации; отменяющий таймаут и `Failed(ReceiverTimeout)` удалены; процесс можно уничтожить сразу после `finish()` (`I6-D26`, `I6-D29`, `I6-D30`, `I6-P4`, `I6-P8`, `I6-N6`, `I6-M7`); «Да, в 9:00» — один подтверждаемый переход DataStore `acceptReminderPrompt` (`notificationPromptShown`, 09:00, `reminderEnabled`) до любого эффекта, шаг продолжения в `SavedStateHandle`, таблица восстановления после смерти процесса, недоступность уведомлений не отбрасывает согласие (`I6-D2`, `I6-D40`, `I6-V15`, `I6-P7`); синхронизированы §8.3, §9.1, §9.3–9.5, PR 6B, риски, rollback, план синхронизации документов. Статус — готова к архитектурному ревью. По итогам ревью (2026-09-15) без изменения содержания: документ архитектурно утверждён, решения владельца `O6-1`…`O6-6` подтверждены в рекомендованных вариантах, статусные места обновлены |

---

**Статус документа: ревизия 1.2, архитектурно утверждена, 2026-09-15.** Решения `I6-D*` приняты; решения владельца `O6-1`…`O6-6` подтверждены владельцем 2026-09-15.

**Статус реализации на 2026-09-17: PR 6A и 6B влиты, 6C и 6D не начаты.**

- **PR 6A** — реализован (итерация 6, merge `3570846`).
- **PR 6B** — реализован. Отклонения от плана раздела 13.2 и их причины:
  - строки-предложения 8.6 (`settings_reminder_time`, `settings_reminder_time_value`, `cd_settings_reminder_time`) приняты в предложенных значениях — «Время», `%1$d:%2$02d`, «Время напоминания, %1$s»;
  - иконка уведомления `ic_stat_reminder` создана **в самом PR 6B** как оригинальный ассет проекта (`original project asset, created for PoPoRyadku`), а не получена входными данными: внешнего источника и сторонней лицензии у неё нет, происхождение зафиксировано в `VERSIONS.md`;
  - версия WorkManager перепроверена 2026-09-16 по официальной странице релизов и **не изменилась** (2.11.2);
  - `SettingMutation.DragHintSeen` из **I6-D39** в 6B не добавлена: её единственный потребитель — `DragEducationHint` из 6C;
  - `androidTest/…/ReminderWorkSchedulingTest` содержит `I6-N6`; уничтожение процесса после `finish()` проверено вручную (`I6-M7`), потому что тест инструментации не может убить собственный процесс и продолжить проверку;
  - `AppNavHostTest` (навигация) дополнен закрытием предложения напоминания: после первого завершённого дня диалог модален, и системная «назад» закрывает сначала его. Утверждение теста не ослаблено.
- **6C и 6D не начаты**; итерация 6 **не завершена**.

`I4-C6`, `I5-M5`, `I5-M6`, `I5-M10` не выполнены и остаются пунктами release-readiness итерации 7, как и физические проверки итерации 6 (`I6-M2`, `I6-M4`, `I6-M10`, физические части `I6-M5`, `I6-M6`, `I6-M11`, `I6-M14`, `I6-M15`).
