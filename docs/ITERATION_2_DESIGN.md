...
| 2C | Debug-экран забудут удалить в итерации 3 | Инструмент доживает до релиза | Весь код итерации 2 — в каталоге `app/src/debug/java/ru/poporyadku/debug/` и одном манифесте; удаление сводится к `rm -r` двух путей. Ничего снимать в `src/main` не нужно: продуктовый код debug-швов не содержит (**D-16**, **D-21**), а `DebugClockProvider` остаётся как штатная debug-реализация часов. В критериях приёмки итерации 3 пункт «удалить debug-экран» уже есть |
