...
Итерация 5, PR 5E. Входные данные владельца **O5-7**, предоставлены 2026-09-12; без файлов с подтверждённой лицензией PR 5E не мержится (`ITERATION_5_DESIGN.md`, §3.14, §13.2).
...
| Автор | Kenney |
| Пакет | UI Audio |
| Источник | [kenney.nl/assets/ui-audio](https://kenney.nl/assets/ui-audio) |
| Лицензия | Creative Commons CC0 1.0 (Public Domain Dedication) |
| Дата получения | 2026-09-12 |

| Файл проекта | Исходный файл пакета |
| --- | --- |
| `app/src/main/res/raw/feedback_move.ogg` | `click5.ogg` |
| `app/src/main/res/raw/feedback_accept.ogg` | `rollover5.ogg` |

CC0 не требует указания авторства, но происхождение фиксируется здесь намеренно: без записи об источнике проверить право на использование файла нельзя, а «звук взят откуда-то» — не состояние, с которым публикуются в магазины.

### Выполненная обработка

Владелец выполнил над исходными файлами:

- выбор и обрезка (`selected and trimmed`);
- преобразование в моно, 48 кГц, OGG Vorbis;
- снижение усиления ниже −12 dBFS;
- добавление нарастания и спада (fade-in, fade-out) — чтобы на краях не было щелчка.

Перекодирование в проекте **не** выполняется: файлы лежат в `res/raw` ровно такими, какими их отдал владелец, и ни один шаг сборки их не трогает.

### Фактические параметры

| Файл | Формат | Каналы | Частота | Длительность | Размер | Пик |
| --- | --- | --- | --- | --- | --- | --- |
| `feedback_move.ogg` | OGG Vorbis | моно | 48 кГц | 37,7 мс | 4293 байта | −14,7 dBFS |
| `feedback_accept.ogg` | OGG Vorbis | моно | 48 кГц | 111,8 мс | 4952 байта | −13,9 dBFS |

Спецификация `ITERATION_5_DESIGN.md` §3.14 выполнена по всем пунктам: `feedback_move` ≤ 40 мс, `feedback_accept` ≤ 120 мс, пик ≤ −12 dBFS, каждый файл ≤ 20 КБ. `feedback_accept` — мягкий нейтральный тон без мелодии и «победной» окраски (`DESIGN_PRINCIPLES.md` §9: победных звуков нет).

### Контрольные суммы

```text
feedback_move.ogg    5553ba971f66b6a46d0582c3f2e7da4a512d4c766d7289912cd58e2052ab70da
feedback_accept.ogg  949cf38fb518fe95ff294e4b497951c68588608f44e51271fbf7418ed2f918b3
```
