    ...
    data/
      ...
      db/
        ...
        dao/                    // PuzzleDao, DailySetDao, AssignmentDao, AttemptDao, DayResultDao,
                                // ArchiveDao — read-only: day_results ⋈ day_assignments, keyset (ADR-017)
        entity/
...
### Архив и статистика (итерация 5)

Архив и статистика не добавляют ни таблиц, ни колонок, ни индексов, ни сохранённых агрегатов: схема остаётся версии 1 (`ITERATION_5_DESIGN.md`, **I5-D2**, **I5-D3**).

- **Архив — это `day_results ⋈ day_assignments`** по `local_date`: внутреннее соединение по первичным ключам обеих таблиц в read-only `ArchiveDao`, одна строка на дату. День попадает в архив тогда и только тогда, когда у него есть строка `day_results`, то есть хотя бы одна попытка: незавершённый день (в том числе сегодняшний) показывается с частичным счётом, пропуск — закрытый слот с нулём, отложенное назначение без попыток в архив не попадает. Номер дня — `set_index + 1` назначения. Фильтра по `pack_id` нет: история — дни пользователя, а не пакета. Строка `day_results` без назначения — нарушение инварианта записи: соединение её не показывает, статистика учитывает (тест `I5-A5`).
- **Keyset-окно, а не `OFFSET`** (ADR-017): разведка `LIMIT 51` строго раньше курсора двигает нижнюю границу, а показываемые строки — наблюдаемое окно `local_date >= граница` (после последней страницы — все дни). Строка отображается в `ArchiveDay` строгим mapper'ом: неразбираемая дата, `set_index ∉ 0 until Int.MAX_VALUE`, `completed_count ∉ 1..3`, `total_score ∉ 0..6 × completed_count` или `is_complete ≠ (completed_count == 3)` бросают исключение до построения модели; значения не обрезаются.
- **Единый источник статистики** — чистая функция `StatisticsCalculator.of(days, today)` над одной выборкой `day_results`. Её вызывают и архив (`GetStatisticsUseCase` поверх `ProgressRepository.observeDayResults()`), и Home (`GetTodayStateUseCase` строит из результата `TodayStats`), поэтому экраны не расходятся по построению. SQL-агрегата статистики в продуктовом коде нет. Точная семантика (`ITERATION_5_DESIGN.md`, §3.5, **I5-D5**; решение владельца O5-3):

| Показатель | Определение |
| --- | --- |
| Сыграно дней | все строки `day_results`, включая незавершённые |
| Средний балл из 18 | только завершённые дни: `S / n`, где `S` — сумма `total_score`, `n` — их число; одна десятичная, half-up от точного значения, целочисленно: `(20·S + n) div (2·n)` десятых. Незавершённый день не входит ни в числитель, ни в знаменатель; при `n = 0` значения нет |
| Лучший день | максимум `total_score` по всем сыгранным дням, включая незавершённые; 0 при пустой истории |
...
| Проверка настоящего пакета | `python3 tools/validate-content/validate.py app/src/main/assets/content --expect-sets 35 --expect-puzzles 105` | 4C-1; `--expect-*` — 4D |
...
**Настоящий пакет проверяется на каждом прогоне CI.** Шаг «Validate content pack» появился в PR 4C-1 вместе с самим каталогом assets и проверял батчи без `--expect-*`; с PR 4D флаги `--expect-sets 35 --expect-puzzles 105` — постоянный критерий релиза (`ITERATION_4_DESIGN.md`, **I4-D21**): любое уменьшение пакета роняет сборку кодом `M10_EXPECTED_VOLUME`. Self-check по-прежнему прогоняет валидатор на синтетических фикстурах: 21 правило, буквенные подкоды и `M01`–`M10`.
...
### ADR-016. Предрелизный hard cutover с временной фикстуры

**Контекст.** В итерации 3 приложение играло на временной фикстуре: три головоломки литералами в коде (`data/content/temporary/`) и три набора в `daily_sets` пакета `core-ru` с `setIndex = 0..2`. Настоящий пакет итерации 4 занимает тот же `packId` и те же индексы, но с другими составами. На устройствах, где временные дни уже сыграны, в `day_assignments`, `puzzle_attempts` и `day_results` лежит история по наборам, которых в настоящем пакете нет. Временная фикстура публично не распространялась и никому не передавалась с обещанием сохранить прогресс — это подтвердил владелец проекта 2026-09-03 (`ITERATION_4_DESIGN.md`, **I4-D3**).

**Решение.** Переход выполняется как предрелизный hard cutover (PR 4D):

1. **Legacy-пакет не создаётся.** Ни второго `packId`, ни сохранения payload временных головоломок, ни «переезда» истории: совместимость с фикстурой никому не обещалась. Каталог `data/content/temporary/` удалён целиком, продуктовый граф связан с `ContentImporter` и `PuzzleRepositoryImpl`.
2. **Автоматическая потеря прогресса запрещена.** Импортёр никогда не удаляет и не изменяет `day_assignments`, `puzzle_attempts`, `day_results` и не «чинит» расхождение за пользователя.
3. **Несовместимое локальное состояние даёт `ContentConflict`.** Назначение вне диапазона пакета, изменившийся состав назначенного набора или сыгранный `puzzleId`, чужой своему слоту, — это `ContentInstallException.Conflict`, брошенный внутри транзакции: ни одна таблица и отметка в DataStore не изменены. Home показывает `Error` с «Повторить».
4. **Сброс — только по явному подтверждению человека и только в debug.** Действие «Сбросить контент» (`ContentResetAction` → `ContentReset`, `src/debug`) предлагается исключительно при `ContentConflict`, открывает диалог, прямо говорящий об удалении всего локального прогресса, и очищает базу целиком (`clearAllTables()`), не трогая настройки. Следующий `ensureInstalled()` импортирует настоящий пакет: ранний выход без подтверждения базой невозможен (ADR-015, `ITERATION_4_DESIGN.md` §10.4). В release-сборке набор действий восстановления пуст (**I3-D47**).

**Альтернативы.** (а) Legacy-пакет `core-ru-legacy` с сохранением payload временных головоломок — ничей прогресс не ломается, но второй пакет остаётся в коде навсегда ради нуля пользователей. (б) Автоматически удалять конфликтующие назначения — пользователь не увидел бы ошибки ценой молчаливого стирания истории; прямо запрещено **I3-D47** и **I4-D3**. (в) Молча подменить составы уже сыгранных дней — архив показал бы на сыгранных днях чужие головоломки.

**Последствия.** Плюс: один пакет, одна модель контента, никакого кода миграции ради фикстуры. Минус: на устройстве разработчика, где игрались временные дни, первый запуск сборки PR 4D показывает `Home.Error` — ожидаемо, это единственный сценарий ручной приёмки «конфликт → сброс → первый запуск» (`I4-E3`). Механика обнаружения конфликта от этого выбора не зависит, поэтому решение обратимо: при отрицательном ответе владельца потребовался бы legacy-пакет, но не переделка импортёра.

### ADR-017. Архив: keyset-окно без Paging 3

**Контекст.** Архив читает базу порциями по 50 дней с автоматической подгрузкой при прокрутке (`UX_FLOW.md` §7), а первая страница обязана обновляться, когда пишется сегодняшний результат. Новая строка `day_results` может появиться только на дате отложенного назначения, а оно всегда самое позднее, поэтому новый день всегда оказывается сверху списка. Paging 3 в зависимостях нет, за год игры в архиве до 365 строк.

**Решение.** Keyset по `local_date DESC` (`ITERATION_5_DESIGN.md`, §3.4, **I5-D4**). Разведка страницы — `LIMIT 51` строго раньше курсора (`local_date < :before`; для первой страницы — самые поздние дни): 51-я строка — единственный признак продолжения, новая нижняя граница — дата 50-й строки. Показываемые строки берутся не из разведки, а из наблюдаемого Room-запроса окна `local_date >= нижняя граница`; когда продолжения нет, наблюдаются все дни. Окно ограничено датой, а не числом строк, инвалидируется записью в `day_results` и `day_assignments` и только растёт. Запросы — `ArchiveDao`, разведка и граница — `GetArchiveUseCase`.
...
1. **Внешние действия — только за `ExternalApps`** (`ui/platform`): `canViewUrl`/`viewUrl`, `canComposeEmail`/`composeEmail`, `shareText`, без Android-типов в сигнатурах. Единственная реализация `AndroidExternalApps` — единственное место `src/main`, где создаются и запускаются внешние `Intent`: перед запуском проверяет обработчик (`resolveActivity`; на API 30+ видимость дают `<queries>` манифеста — `VIEW http/https`, `SENDTO mailto`); отказ `PackageManager` при разрешении — «обработчика нет» для `can*` (они вызываются из композиции) и `Failed` для запуска; при запуске перехватывает `ActivityNotFoundException` → `NoHandler`, `SecurityException` и прочие отказы → `Failed`; исключения наружу не выпускает ни на одном шаге. `shareText` — исключение из проверки обработчика: `Intent.createChooser` поверх `ACTION_SEND` с `text/plain` разрешается системой всегда, `<queries>` и разрешений не требует, а `ActivityNotFoundException` у него означает отказ платформы и даёт `Failed`, а не `NoHandler`. Второй запуск раньше возврата на экран (`ON_RESUME`) и раньше 1000 мс — `Suppressed`; время фиксирует только успешный `startActivity`. Экземпляр один на Activity: его создаёт `MainActivity` и отдаёт экранам через `LocalExternalApps`; тесты подставляют фейк. ViewModel отдаёт только данные эффектом, запускает route-контейнер при сборе эффекта.
